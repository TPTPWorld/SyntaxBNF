...
{ int i, d;
  ...
  if (ss == NULL) return;
  if (ss->ch[1] != 0) printf("<div style=\"margin-left:10px;margin-right:10px;border:1px solid #000000;\">\n");
  for (d = 0; d < depth; d++) printf("&nbsp;&nbsp;");
  if (ss->ch[0] == 0) 
    if (strcmp(ss->sym, "null") != 0)
    {symbol=strdup(ss->sym);
     buf=strchr(symbol,' ');
     if (buf != NULL)
     {
      *buf=0;buf++;
      printf("%s <span style=\"font-family:monospace; color:#990000;\">%s</span><br/>",symbol,buf);
     }
     else printf("%s<br/>\n", ss->sym);
    }
    else printf("%s<br/>\n", ss->sym);
  else if (ss->ch[1] == 0) printf("&lt;%s&gt;<br/>",ss->sym);
      else { printf("<a style=\"font-family:monospace;color:#990000;\" onclick=\"javascript:document.getElementById('tree%d').style.display=(document.getElementById('tree%d').style.display=='none')?'block':'none';\">", treediv, treediv);
             mPrintTree(ss, depth);
             printf("</a><br/>");

            printf("<div id=\"tree%d\" style=\"display:none;\">\n",treediv);
            for (d = 0; d < depth; d++) printf("&nbsp;&nbsp;");
            printf("&lt;%s&gt;<br/>", ss->sym, treediv);
//            printf("&lt;%s&gt;<input type=\"checkbox\"  onclick=\"document.getElementById('tree%d').style.visibility=this.checked?'visible':'hidden';\">", ss->sym, treediv);
//            printf("<div id=\"tree%d\" style=\"visibility:hidden;\">\n",treediv);
            treediv++;
      }
  if (strcmp(ss->sym, "PERIOD .") == 0)
    {nPrintIdx = (ss->symIdx+1)%tptp_store_size;}
//    {nPrintIdx = (ss->symIdx+1)%tptp_store_size; printf("\n<br/>\n");}
  if (ss->symIdx >= 0)
    nPrintComments((ss->symIdx+1)%tptp_store_size, depth);
  i = 0;
  while(ss->ch[i] != 0) {nPrintTree(ss->ch[i], depth+1); i++;}
  if (ss->ch[1] != 0) printf("</div></div>");
  //printf("</div>\n");
  return; }
...
//    {printf("\n");for (d = 0; d < depth; d++) printf(" ");depth--;}
  ...
  if (strcmp(ss->sym, "PERIOD .") == 0)
  { mPrintIdx = (ss->symIdx+1)%tptp_store_size; printf("<br/>\n"); }
  if (ss->symIdx >= 0)
    mPrintComments((ss->symIdx+1)%tptp_store_size, depth);
  i = 0;
...
void doubleFeature(mTree ss, int depth)
{ if (ss != NULL) {
  printf("<div style=\"border:1px solid #000000;padding:10px;\">\n");
  printf("<p style=\"font-weight:bold;\">formula:</p>");
  printf("<span style=\"font-family:monospace\">");
  mPrintTree(ss, depth);
  printf("</span>");
  printf("<br/>\n");
  printf("<p style=\"font-weight:bold;\">explore:</p>");
  nPrintTree(ss, depth);
  printf("</div><br/>\n");
  }
}
